...
public class UpdateDayEventCommandHandler(IDayInfoWriteRepository dayInfoWriteRepository)
    : IRequestHandler<UpdateDayEventCommand>
{
    public Task Handle(UpdateDayEventCommand request, CancellationToken cancellationToken)
    {
        return dayInfoWriteRepository.UpdateDayEvent(
            request.EventId,
            request.Timestamp,
            request.Title,
            request.Description);
